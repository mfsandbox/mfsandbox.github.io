...
const getDotInfos = (cardCount, cardIndex, dotRadiusMap) => {
    ...
    if (cardCount < perspectiveThreshold) {
        return _.range(leftMostIndex, rightMostIndex + 1).map((dotIndex) => {
            return {radius: dotRadiusMap[0], current: (dotIndex == cardIndex)};
        })
    }
...
const getDotCircles = (cardCount, cardIndex, dotRGB) => {
    ...
    return dotInfos.map(function (dotInfo, dotIndex) {
        return `<circle cy = '15px' cx = '${dotArrayCenters[dotIndex]}px' r = '${dotInfo.radius}px' fill='${dotInfo.current ? currentRGBA : otherRGBA}'></circle>`;
    })
}

const getDotPanelSvg = (cardCount, cardIndex, dotRGB) => {

    return `<svg height=30px width=200px>${getDotCircles(cardCount, cardIndex, dotRGB)}</svg>`
}

const drawNavDots = (cardCount, cardIndex) => {
    let currentCard = cards[cardIndex];

    let dotRGB = currentCard.classList.contains("dots-light") ? {red: 255, green: 255, blue: 255} : {red: 1, green: 31, blue: 116};
...
hammered.on('swiperight', function (event) {
    decrementCardIndex();
    setCardVisibility(cards, navIcons)
});
hammered.on('swipeleft', function (event) {
    incrementCardIndex();
    setCardVisibility(cards, navIcons)
});
...
//initFlipCards();
  ...
  if (cardCount > 1)
  {
      document.querySelectorAll(".cardIndexDecrementor").forEach(
          (decrementor) =>
          {
              decrementor.addEventListener('click', function(event) {decrementCardIndex(); setCardVisibility(cards, navIcons);})
          }
      )
      document.querySelectorAll(".cardIndexIncrementor").forEach(
            (incrementor) =>
            {
